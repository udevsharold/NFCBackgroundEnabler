...
static float protectionTime = 2;
static float debounceTime = 1;
static BOOL airplaneOverride = false;
static BOOL tagLockEnable = YES;
...
- (void)handleDetectedTags:(NSMutableArray <NSObject<NFTag> *> *)tags {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/NFCActivator.dylib"]){
        [[%c(NFCActivatorClient) sharedInstance] manager:self nfcActivatorDetectedTags:tags];
    }
}
